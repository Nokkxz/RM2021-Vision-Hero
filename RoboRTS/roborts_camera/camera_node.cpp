...
CameraNode::CameraNode() {
  ...
  watchdog_camera_pub_ = nh_.advertise<std_msgs::Int8>("watchdog_camera", 10);
  watchdog_camera_timer_ = nh_.createTimer(ros::Duration(1), &CameraNode::timerCallback, this);
...
{
  // ROS_INFO("camera node send: %d", watchdog_camera_msg_.data);
  watchdog_camera_pub_.publish(watchdog_camera_msg_);
  watchdog_camera_msg_.data = 0;
}
