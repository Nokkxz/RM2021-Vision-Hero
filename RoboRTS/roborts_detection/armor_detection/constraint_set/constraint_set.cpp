...
void ConstraintSet::LoadParam() {
  ...
  while ((get_intrinsic_state < 0) || (get_distortion_state < 0)) {
    ...
    ROS_INFO("lxy_: %d",intrinsic_matrix_.size().height);
...
ArmorInfo ConstraintSet::SlectFinalArmor(std::vector<ArmorInfo> &armors) {
  std::sort(armors.begin(),
            ...
            [](const ArmorInfo &p1, const ArmorInfo &p2) {
              // float width = 1280;
              // float height = 1024;
              // float dx1 = p1.rect.center.x - width/2;
              // float dy1 = p1.rect.center.y - height/2;
              // float dx2 = p2.rect.center.x - width/2;
              // float dy2 = p2.rect.center.y - height/2;
